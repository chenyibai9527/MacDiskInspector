# 更新日志

本项目遵循 [Semantic Versioning](https://semver.org/)；候选版本在版本号后使用
`-rcN`，公开预览版本使用 `-preview.N`。

## 未发布

### 修复

- 让主内容背景贯穿全尺寸窗口并隐藏工具栏的独立材质层，消除深色模式下标题栏与主体的色差；同时移除标题栏分隔线，浅色与深色外观保持一致。

### 工程

- 新增统一的手动发布工作流，一次完成版本校验、测试、Universal DMG 构建、Release 发布和官网部署验证。
- GitHub Actions 的检出与 Artifact 上传组件升级到 Node.js 24 版本，移除 Node.js 20 弃用警告。
- 本地构建完成后自动从 LaunchServices 注销构建产物，避免开发版本与正式安装版同时出现在 App 启动器中。

## [0.2.3-preview.1] - 2026-07-26

### 安全修复

- 补充保护 `~/Library` 中的照片分析、媒体索引以及 Apple Music、iTunes 相关缓存，避免重新扫描时再次触发照片或媒体资料库授权。
- 用户未主动开启媒体扫描范围时，重新扫描会继续跳过这些受保护目录，不会因历史设置而意外放行。

### 验证

- 新增重新扫描配置回归测试；39 项单元测试、核心验收、源码安全检查和 Universal DMG 验证通过。

## [0.2.2-preview.2] - 2026-07-26

### 修复

- 统一浅色、深色和自动外观下的窗口结构，标题栏与主体使用连续的系统自适应背景。
- 移除只在浅色模式出现的装饰渐变和工具栏强制不透明背景，保留侧栏原生材质层级。

### 验证

- 在 macOS 26 的自动、浅色和深色外观下实际运行验收。

## [0.2.2-preview.1] - 2026-07-26

### 安全修复

- 目录扫描改用逐项读取名称，并在读取子项元数据或打开子目录之前应用排除规则，避免 Foundation 预取受保护目录信息而触发系统隐私授权。
- 默认保护范围补充日历、通讯录、提醒事项和家庭数据，并继续覆盖照片、音乐及其他 App 数据。

### 工程

- 将本地 Xcode 构建缓存迁移到 `build/DerivedData.noindex`，防止开发构建的 `.app` 被 Spotlight 和 App 启动器当作第二份已安装应用。

### 验证

- 新增目录打开与元数据读取观测测试；38 项单元测试、源码安全检查和 Universal DMG 验证通过。

## [0.2.1-preview.1] - 2026-07-26

### 安全修复

- 将递归目录枚举改为逐层浅枚举，在读取子目录内容或文件元数据之前先应用受保护目录规则，避免全盘扫描仅因经过音乐、照片等目录就触发 macOS 权限询问。
- 全盘扫描会为本机所有普通用户的个人目录建立默认保护规则，而不再只覆盖当前登录账户；当前账户的设置不会放行其他账户。

### 验证

- 新增可观测目录读取测试，明确验证被排除的音乐目录从未被打开，而不只是验证其中的文件没有计入结果。
- 现有符号链接、硬链接、权限错误、取消、部分结果、大目录和安全命令测试全部通过。

## [0.2.0-preview.1] - 2026-07-26

### 修复

- 侧栏拖动宽度限制在适合导航的范围内，避免过窄难读或过宽挤压主体。
- 扫描过程中，工具栏不再只显示无文案的旋转指示器，而是明确显示“正在扫描”和“取消”。
- 全盘扫描默认跳过其他 App 的 `Containers` 与 `Group Containers`，避免在用户未主动选择时触发“访问其他 App 数据”的系统授权提示。
- 修复全盘扫描通过 `/System/Volumes/Data` 镜像路径进入音乐、照片等默认排除目录，并意外触发 macOS 隐私权限询问的问题。
- 将 Verification 与 Benchmark 的入口文件从特殊文件名 `main.swift` 改为普通 Swift 文件名，避免 Xcode 的 Swift Package scheme 将隐式入口和 `@main` 同时编译而报错。
- 修复长时间扫描后趋势图只显示在卡片右侧的问题；采样现在压缩保留整个扫描过程，并使用稳定的完整横轴。

### 新增

- 点击取消会立即停止继续读取，再让用户选择查看已完成部分或放弃结果；部分报告会醒目标记为不完整，避免误判为全目录统计。
- 扫描完成后，概览页和工具栏提供“重新扫描”与“选择其他目录”，无需猜测下一步入口。
- 设置中新增“其他 App 数据”受保护范围；开启后才会扫描微信等 App 容器，直接选择时仍可只对本次放行。
- 使用说明新增“频繁扫描会伤硬盘吗”，说明 SSD 读取耐久与全盘扫描的资源占用。
- 默认跳过受保护个人目录，并允许用户在设置中逐项开启或仅对一次扫描放行。
- 扩展系统、开发、设计、浏览器、云盘、媒体资料库和常见应用的高占用解释规则。
- 为 Library、Application Support、Containers、Xcode、Docker、Homebrew、Node.js、Adobe、Figma、主流浏览器等提供来源、风险与建议。

## [0.2.0-rc5] - 2026-07-24

### 改进

- 重建窗口级 Liquid Glass 层级：增加可感知的环境背景、悬浮扫描操作层与侧栏隐私层。
- 扫描范围、只读状态和主要操作现在集中显示在同一功能层中，不再依赖零散的小玻璃按钮。
- 页面切换使用无弹跳的弹簧过渡；排序选中态使用 `glassEffectID` 和 matched glass transition。
- “减少动态效果”和“降低透明度”开启时自动切换为更克制的呈现。
- 删除标题栏中与扫描操作层重复的入口。

### 验证

- 使用项目内测试夹具在 macOS 26 上实际运行并检查空状态、扫描结果、占用排行、排序切换和使用说明。

## [0.2.0-rc4] - 2026-07-24

### 新增

- 在 macOS 26 及以上使用原生 SwiftUI Liquid Glass。
- 占用排行排序器使用 `GlassEffectContainer`、玻璃按钮和选中态突出样式。
- 主要操作、详情操作和临时状态提示使用系统 Liquid Glass。
- macOS 13–15 保留原有标准材质和按钮样式。

### 工程

- GitHub Actions 切换至 macOS 26 runner，确保使用包含 Liquid Glass API 的 Xcode 26 SDK。

## [0.2.0-rc3] - 2026-07-24

### 修复

- “占用排行”切换排序时不再重复计算与排序无关的建议列表。
- “类型”和“风险”排序现在显示清晰的分组标题，并说明当前排序规则。
- 为大小、类型和风险排序加入确定性的次级排序规则与单元测试。

## [0.2.0-rc2] - 2026-07-24

### 修复

- “使用说明”页面的卡片现在保持等宽，不再随文字长度变化。
- 重写应用内主要中文说明，移除中英文混写和生硬的翻译腔。
- 将“候选空间”等内部术语改成更容易理解的用户表达。

## [0.2.0-rc1] - 2026-07-24

### 新增

- 原生 SwiftUI 磁盘概览、占用排行、Finding 详情和建议中心。
- 用户显式选择目录后的本地只读扫描，支持取消、覆盖率和权限缺口。
- allocated/logical size、文件数、修改时间、多级目录聚合。
- 符号链接跳过、默认不跨卷、设备号与 inode 硬链接去重。
- npm、Library Caches、Antigravity browser recordings、Cursor 数据库、微信容器、Chrome 本地模型、swapfile 和系统区域规则。
- 固定 allowlist 的检查/清理命令复制，不执行 Shell。
- 空间构成与风险分布可视化、大目录提示和使用说明。
- Universal（arm64 + x86_64）社区预览包与可复现验证脚本。

### 安全

- App Sandbox 仅声明用户选择目录只读权限。
- 无网络 entitlement、遥测、删除、移动、`sudo` 或 privileged helper。
- 高风险和系统管理数据不提供删除命令。

### 已知限制

- 社区包采用 ad-hoc 签名，未经过 Apple Developer ID 签名与公证。
- 首次打开可能需要 Finder 中右键“打开”，不同 macOS 安全策略下表现可能不同。
- Mail、Messages、Safari 等受保护目录的覆盖率取决于用户授予的系统权限。
